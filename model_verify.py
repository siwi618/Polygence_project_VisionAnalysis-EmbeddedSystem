...
def format_result(r: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"validation set size: {r['total']} images",
            "",
            f"{'Model':<10}| {'Val Accuracy':^14}",
            "-" * 10 + "|" + "-" * 16,
            f"{'Keras':<10}|    {r['keras_acc']:.4f}     ({r['keras_correct']}/{r['total']})",
            f"{'TFLite':<10}|    {r['tflite_acc']:.4f}     ({r['tflite_correct']}/{r['total']})",
            "",
            f"val accuracy change (TFLite - Keras): {r['tflite_acc'] - r['keras_acc']:+.4f}",
            f"prediction agreement rate (on val):   {r['agreement']:.4f} ({r['agree_count']}/{r['total']})",
            f"max probability difference (on val):  {r['max_prob_diff']:.6f}",
            f"avg probability difference (on val):  {r['avg_prob_diff']:.6f}",
            "",
        ]
    )
